...
- **Sync engine**: canonical → mirror, with user-edits promoted back. Only `Codex` is a writable mirror target today (Cursor/Codex adapters are stubbed).
- **Session scraping**: reads transcripts from Codex, Codex, and Cursor into `~/.skillset/sessions/` (SQLite). This is the mining input, not a mirror.
...
- `src/core/paths.ts` — well-known paths (`~/.skillset/`, `~/.Codex/skills/`)
...
- `src/core/adapters/` — per-agent mirror targets. Only `Codex` is writable today.
- `src/ingest/` — **forked from nia-cli** (MIT). Folder-walking utilities + per-agent session readers (`sessions/Codex.ts`, `codex.ts`, `cursor.ts`) that land transcripts in a SQLite store via `sessions/writer.ts` and `sessions/sqlite.ts`. Notable delta from upstream: `.jsonl` is in the TEXT_EXTENSIONS allowlist (upstream skips it, which would silently drop Codex conversations).
...
Mining pipeline:
- `skillset scrape` — pull session transcripts from Codex / Codex / Cursor into `~/.skillset/sessions/` (`--source` to pick one, `--full` to ignore incremental cursors)
