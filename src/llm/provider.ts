 ...
 *   - kimi-cli: spawn `kimi -p` (subscription, no API key)
...
import { kimiCliChat, kimiCliIsAvailable } from "./kimi-cli.js";
import { grokCliChat, grokCliIsAvailable } from "./grok-cli.js";
...
export async function chat(
  ...
  switch (config.provider) {
    ...
    case "kimi-cli":
      return kimiCliChat(config, opts);
    case "grok-cli":
      return grokCliChat(config, opts);
    case "anthropic":
...
export async function isAvailable(config: LLMConfig): Promise<{
  ...
  switch (config.provider) {
    ...
    case "kimi-cli":
      return kimiCliIsAvailable(config);
    case "grok-cli":
      return grokCliIsAvailable(config);
    case "anthropic":
